...
function WorkflowManagerEngine(currentUserInformation) {
    ...
    this.SecurityStatus = {
        ...
        before: "You can not currently take action on this request because its not at your stage..",
        after: "You have already taken an action on this request.."
    }
    ...
    */
    ...
    this.routeEngine = function (baseObject) {
        ...
        properties.nextActor = function (initiationCode) {
            ...
            =========================================*/
            properties.updateRoutesinFlow = function (UsersSPObject, callback, isSPFlow, usersInGroupsManual) {
                ...
                for (var i = 0; i < users.length; i++) {
                    var customUser = UsersSPObject[users[i]];
            ...
            properties.PageSecurity = function (pageMode, currentStage, approvalStatus, callback) {
                if (pageMode === baseObject.stages.securityModeTask) {
                    if (this.security(currentStage, approvalStatus)) {
                        callback();
                    } else {
                        var status = this.failedSecurityError(currentStage);
                    ...
                    if (this.security(currentStage, approvalStatus)) {
                        ...
                        callback(baseObject.SecurityStatus.none);
                    }
                }
            },
            ...
            properties.viewSecurity = function (customSetting) {
                var AuthorizedAccess = false;
                var initiatorDetails = this.currentActor(baseObject.stages.initiatorCode);
                if (baseObject.CurrentUserProperties.login == initiatorDetails.authenticationValue) {
                    return true;
                }
                ...
                if (typeof customSetting !== "undefined") {
                    if (Array.isArray(customSetting)) {
                        var isAmong = false;
                        for (var i = 0; i < customSetting.length; i++) {
                            if (customSetting[i] === baseObject.CurrentUserProperties.login) {
                                isAmong = true;
                                break;
                            }
                        }
                ...
                for (var x = 0; x <= (routingGroups.length - 1); x++) {
                    ...
                    for (var y = 0; y <= (possibleRoutes.length - 1); y++) {
                        if (possibleRoutes[y].flow === baseObject.stages.normal) {
                            if (possibleRoutes[y].authenticationType == baseObject.stages.group && possibleRoutes[y].authenticationType) {
                                AuthorizedAccess = true;
                                break;
                            } else if (possibleRoutes[y].authenticationType == baseObject.stages.user && baseObject.CurrentUserProperties.login == possibleRoutes[y].authenticationValue) {
                                AuthorizedAccess = true;
                                break;
                            }
